...
After the completed processing-to-UI separation work, the central relationship looks approximately like this:
...
__main__.py
    ...
    +--> StoryToolkitEngine
          ...
          Tk UI
            ...
            +--> still accesses ToolkitOps directly
...
| **Location**           | Queue update and completion paths in `processing_queue.py`; `ToolkitOps.notify_observers`                                                                   |
| **Current state**      | Queue code still publishes action names such as `update_queue`, `update_queue_item`, queue-item completion names and arbitrary `on_stop_action_name` values |
| **Remaining coupling** | Event meaning is encoded in strings and event payloads do not identify the changed job or its state                                                         |
| **Status**             | Open                                                                                                                                                        |
...
| **Location**           | Queue windows and shutdown handling in `storytoolkitai/ui/toolkit_ui.py`                                                       |
| **Current state**      | Some UI code still accesses `processing_queue`, raw queue dictionaries and queue cancellation methods directly                 |
| **Improvement made**   | `StoryToolkitEngine` exposes copied queue snapshots through `list_jobs(...)` and `get_job(...)`, and exposes `cancel_job(...)` |
| **Remaining coupling** | The existing Tk queue and shutdown paths have not all been migrated to the engine methods                                      |
| **Status**             | In progress                                                                                                                    |
| **Related commit**     | `89eabcf2c6ff3248e5c045f2bd40911d175f12d0`                                                                                     |
...
| **Location**           | Queue-window event registration and refresh code in `toolkit_ui.py`                                                 |
| **Current state**      | The UI still rechecks queue state after registering callbacks because jobs may finish before a listener is attached |
| **Remaining coupling** | Payload-free events and direct queue reads make it difficult to treat engine snapshots as the authoritative state   |
| **Status**             | Open                                                                                                                |
...
| **Improvement made**   | New transcription-event and Resolve-operation paths use the engine                                                                                            |
| **Remaining coupling** | Most legacy UI workflows can still bypass the engine                                                                                                          |
| **Status**             | In progress                                                                                                                                                   |
...
| C07 | Queue uses implicit action strings            | Open                     |
| C08 | Queue depends on complete `ToolkitOps`        | Open                     |
| C09 | UI accesses queue internals                   | In progress              |
| C10 | Queue event timing workarounds                | Open                     |
...
## UI access to queue internals

Matches identify C09 and C10 work.

```bash
rg -n --glob '*.py' \
  'processing_queue|queue_history|queue_threads|queue_tasks|get_all_queue_items|get_item\(|set_to_canceled|cancel_item' \
  storytoolkitai/ui
```
